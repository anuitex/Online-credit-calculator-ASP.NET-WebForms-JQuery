...
$(document).ready(function () {
    _currency = 'dollar';
    $('.deposit-calculator-currency [data-class-currency=fa-dollar-sign] i').addClass('active');
    $('.deposit-calculator-content .selected-currency-span i').addClass('fa-dollar-sign fas');
});


function selectCurrency(e) {
    $('.deposit-calculator-currency').find('.currency-icon').removeClass('active');
    $(e).find('.currency-icon').addClass('active');
    $('.deposit-calculator-content .selected-currency-span i').removeClass().addClass(e.getAttribute('data-class-currency') + ' fas');

    _currency = e.getAttribute('data-currency');
}

function sendOrder() {
    var form = $('#sendDepositWrapper');
    var rateValue = $(form).find('div .money-rate-wrapper .show-money-rate-value-span')[0].textContent;
    var putMoney = $(form).find('div .money-get-quantity-wrapper .show-money-span')[0].textContent;
    var backMoney = $(form).find('div .money-back-quantity-wrapper .show-back-money-span')[0].textContent;
    var monthQuantity = $(form).find('div .selected-month-quantity .selected-month-quantity-output-span')[0].textContent;

    var bankId = $('.bank-id')[0].textContent;
    var userId = $('.hiddenUserId')[0].textContent;
    var bankName = $('.user-calculator #UserContent_bankName')[0].textContent;
    ...
    $.ajax({
        type: "POST",
        url: "DepositRequest.aspx/AddDepositRequest",
...
$(function () {
    ...
    $('.selected-money-quantity').on('change .selected-money-quantity input', function () {
        var selectedMoney = $('.selected-money-quantity input').val();
        $('.show-money-span').text(selectedMoney);
        calculateCreditValue();
    });

    $('.selected-month-quantity').on('change .selected-month-quantity input', function () {

        var selectedMonth = $('.selected-month-quantity input').val();
        $('.show-month-span').text(selectedMonth);
        calculateCreditValue();
    });

    function calculateCreditValue() {
        var rateValue = 0;
        ...
        if (selectedMonth > 12 && selectedMonth <= 24) {
            rateValue = 15;
        }
        if (selectedMonth > 24 && selectedMonth <= 36) {
            rateValue = 17;
        }
        if (selectedMonth > 36 && selectedMonth <= 48) {
            rateValue = 19;
        }

        $('.show-money-rate-value-span').text(rateValue);

        if (selectedMonth === 0 && Number(selectedMoney) === 0) {
            $('.show-money-rate-value-span').text('0');
        }

        var overPaid = (Number(selectedMoney) * rateValue) / 100;
        var backResult = Number(selectedMoney) + overPaid;

        $('.money-back-quantity .show-back-money-span').text(parseFloat(backResult.toFixed(4)));
    }
});

$(function () {
    $(document).on('.selected-money-quantity input', '.selected-money-quantity input[type="range"]', function (e) {
        var output = document.querySelectorAll('.selected-money-quantity output .selected-money-quantity-output-value')[0];
    ...
    $(document).on('.selected-month-quantity input', '.selected-month-quantity input[type="range"]', function (e) {
        var output = document.querySelectorAll('.selected-month-quantity output span')[0];
        output.innerHTML = (e.currentTarget.value);
    });
